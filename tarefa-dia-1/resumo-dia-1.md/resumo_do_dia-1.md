...
De volta ao exiba como as primeiras 5 linhas do texto (indica: o headexibe mais do que as primeiras 5 linhas do texto.

Agora tente exibir todas as linhas que tem a palavra 'programação' no texto.

Desafio opcional: exiba todas as linhas que começam com '-'. Em seguida,iba todas as linhas que não começou com '-'. não google sobre o comando greppara conseguir resolver esse desafio.

Execute o comando pwdno diretório onde se encontra o arquivo resumo-dia-1.md.

Parabéns! Você já sabe manusear o terminal e executar comandos nele. O terminal é uma base de toda pessoa que programa computadores. Nele você pode executar um pequeno código até um sistema super complexo na nuvem configurar. Continue pesquisando por mais comandos no Google e "brinque" de criar e removedor de pastas (comando rm), copiar arquivos (comando cp), mover arquivos (comando mv), etc.

Nos próximos dias, você aprenderá a usar o comando gite também a executar o código que você escreverá em JavaScript com o comando node.